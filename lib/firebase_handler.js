...
class Firebase {
  ...
  updateTimestamp(agentId, conversationId, newTime) {
    return new Promise((resolve, reject) => {
      ...
      // set a new timestamp in database & mark new CSAT
  ...
  /**
   * getCredentials - Retrieves Dialogflow service account
  ...
  /**
   * getCsatTriggerWindow - Retrieve the CSAT trigger window (in minutes)
   * @param {string} agentId The agent ID with Dialogflow credentials associated
   * @return {Promise} Resolves with trigger window
   */
  getCsatTriggerWindow(agentId) {
    return new Promise((resolve, reject) => {
      if (agentId === undefined) {
        reject(new Error('Missing arguments'));
      }
      const db = this.database();
      const ref = db.ref(`admin/agents/${agentId}`);
      // Retrieve csatTriggerWindow from Database
      ref.once('value', (snapshot) => {
        const data = snapshot.val();
        if (!data || data == null || !data.csatTriggerWindow) {
          reject(new Error('No trigger window present.'));
        } else {
          resolve(data.csatTriggerWindow);
        }
      });
    });
  }
